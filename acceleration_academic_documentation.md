# توثيق تقني متقدم: وحدة التسريع (Acceleration Module)
...
تُعد وحدة `acceleration` خط الدفاع الأول، وأهم ركيزة أدائية في مشروع (Enterprise NGFW). تم تصميم هذه الوحدة بالاعتماد على تقنيات النواة المتقدمة (eBPF) ومسار البيانات السريع (XDP - eXpress Data Path)، لتمكين النظام من التقاط، وتحليل، وتوجيه الحزم الشبكية (Packets) على مستوى كرت الشبكة (NIC Driver Level) وقبل أن تصل إلى بيئة نظام التشغيل (Linux Networking Stack).

**الهدف الأساسي:** تحقيق أداء يُقاس بسرعة السلك (Wire-Speed Processing) بزمن انتقال (Latency) يقدر بالميكروثانية، لعزل الهجمات الموزعة (DDoS)، وتطبيق الفلترة المتقدمة دون إرهاق المعالج المركزي (CPU).
